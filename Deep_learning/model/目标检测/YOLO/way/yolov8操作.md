...
# 训练
```bash
yolo task=detect mode=train model=ultralytics/cfg/models/v8/yolov8.yaml data=ultralytics/models/yolo/Fair/mydata.yaml epochs=600 imgsz=640 resume=True batch=4
```
## 调库训练
```python
...
## 继续训练
```bash
yolo detect train data=ultralytics/models/yolo/Fair/mydata.yaml model=last.pt  epochs=100 imgsz=640 
```
- `resume=True`不加因为原训练已满加，未满加上
### 结果
...
* voc格式训不起来直接串成COCO
	* [函数](Deep_learning/model/目标检测/YOLO/way/VOC_TO_COCO.py)
* yolo的初始配置如储存位置什么的，在`.config`
# 结果文件
查看
```bash
yolo val model=best.pt data=mydata.yaml
```